...
if __name__ == "__main__":
    ...
    scale = 3.5
    barrier = 10
    Vsmooth = barrier*4.*((x/scale)**4 - (x/scale)**2)
    ...
    all_avg_rough_times = np.zeros((ndEs, keep), float)
    all_std_rough_times = np.zeros((ndEs, keep), float)
    for n in range(ndEs):
        dE = dEs[n]
        c_val = float(n)/ndEs
        print dE
...
#    for n in range(ndEs):
#        c_val = float(n)/ndEs
#        plt.plot(all_avg_rough_times[n,:]/all_avg_rough_times[n,0], 'o', ms=10, c=cm.spectral(c_val))
#    plt.plot(smooth_timescales/smooth_timescales[0], 'ko')
#    plt.ylabel("Implied timescales")
#    plt.semilogy()
#    plt.legend()
